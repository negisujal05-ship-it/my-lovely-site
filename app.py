...
def image_watermark_box(image_path, title, text):
...
<img src="https://raw.githubusercontent.com/negisujal05-ship-it/my-lovely-site/main/{image_path}" class="image-bg">
...
# --- PAGE 2 ---
    ...
    image_watermark_box(
        "images/page2.JPG",
        "🌌 Our Cosmic Universe 🌌",
        "Har pal ek naya sitara banata hai."
    )

    st.markdown('''<div class="stylish-box">
...
# --- PAGE 4 ---
    ...
    image_watermark_box(
        "images/page4.JPG",
        "❤️ The Ultimate Destination ❤️",
        "Yahin milta hai sukoon."
    )
    st.markdown('''<div class="stylish-box">
...
<p style="font-size: 18px; color: #e0e0e0; font-style: italic; margin-bottom: 30px;">"Jab jivan mein sachay aur pavitra prem ka agaman hota hai, toh mann ke saare vikar (vices) khud-ba-khud vileen ho jaate hain..."</p>''', unsafe_allow_html=True)

    # 1. DAR / FEAR Block
    st.markdown('<div class="vikar-box">', unsafe_allow_html=True)
    st.markdown('<h4 style="color: #ffffff; margin: 0 0 8px 0; font-size: 20px;">1. डर (Bhay) — <span style="color: #ffb3c1;">Fear</span></h4>', unsafe_allow_html=True)
    st.markdown('<p style="color: #dfdfdf; font-size: 16px; margin: 0; line-height: 1.5;">Sacha prem har tarah ke darr ko mita deta hai. Jab aap kisi se nishwarth prem karte hain, toh khone ka ya zamane ka koi bhay nahi rehta, kyunki prem aatma ko nirbhay (fearless) bana deta hai.</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # 2. MOH / ATTACHMENT Block
    st.markdown('<div class="vikar-box">', unsafe_allow_html=True)
    st.markdown('<h4 style="color: #ffffff; margin: 0 0 8px 0; font-size: 20px;">2. मोह (Moh) — <span style="color: #ffb3c1;">Attachment</span></h4>', unsafe_allow_html=True)
    st.markdown('<p style="color: #dfdfdf; font-size: 16px; margin: 0; line-height: 1.5;">Moh insaan ko bandhan mein baandhta hai aur swarthi banata hai, jisse sacha prem azaad karna sikhata hai. Prem mein \'paane\' ki zidd nahi, balki samarpit hone ka sukoon hota hai.</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # 3. KRODH / ANGER Block
    st.markdown('<div class="vikar-box">', unsafe_allow_html=True)
    st.markdown('<h4 style="color: #ffffff; margin: 0 0 8px 0; font-size: 20px;">3. क्रोध (Krodh) — <span style="color: #ffb3c1;">Anger</span></h4>', unsafe_allow_html=True)
    st.markdown('<p style="color: #dfdfdf; font-size: 16px; margin: 0; line-height: 1.5;">Krodh mann ki agni hai jo sab kuch jala deti hai. Par jahan sacha prem hota hai, wahan krodh ke liye koi jagah nahi hoti; prem us agni par kshama (forgiveness) aur shanti ka thanda jal chhidak deta hai.</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # 4. IRSHYA / JEALOUSY Block
    st.markdown('<div class="vikar-box">', unsafe_allow_html=True)
    st.markdown('<h4 style="color: #ffffff; margin: 0 0 8px 0; font-size: 20px;">4. ईर्ष्या (Irshya) — <span style="color: #ffb3c1;">Zero Jealousy</span></h4>', unsafe_allow_html=True)
    st.markdown('<p style="color: #dfdfdf; font-size: 16px; margin: 0; line-height: 1.5;">Irshya tab paida hoti hai jab hum doosron se tulna karte hain. Sacha prem hume doosron ki khushi mein apni khushi dekhna sikhata hai, jisse jalan poori tarah se mit jaati hai.</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # 5. AHANKAR / EGO Block
    st.markdown('<div class="vikar-box">', unsafe_allow_html=True)
    st.markdown('<h4 style="color: #ffffff; margin: 0 0 8px 0; font-size: 20px;">5. अहंकार (Ahankar) — <span style="color: #ffb3c1;">Ego</span></h4>', unsafe_allow_html=True)
    st.markdown('<p style="color: #dfdfdf; font-size: 16px; margin: 0; line-height: 1.5;">Ahankar kehta hai \'Main sabsay upar hoon\', lekin prem kehta hai \'Main toh kuch bhi nahi\'. Prem mein \'Main\' (Ego) mit jata hai aur sirf \'Hum\' baaki reh jata hai. Sacha prem ahankar ko poori tarah shunya kar deta hai.</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<p style="font-size: 16px; color: #ff6b8b; font-weight: bold; text-align: center; margin-top: 25px;">✨ Jo Prem Ko Chunta Hai, Wo Saare Vikaro Se Mukt Ho Jata Hai ✨</p>', unsafe_allow_html=True)
...
# --- PAGE 8 (Final Page) ---
    ...
    st.markdown('''<div class="stylish-box" style="border: 2px solid #ff6b8b;">
